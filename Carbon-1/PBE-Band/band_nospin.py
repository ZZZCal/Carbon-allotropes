...
def plot_all(title):
    ...
    fig, (axe, ax1) = plt.subplots(1, 2, sharex=False, sharey=True, figsize=(12, 8))
    ...
    colormaps='blue'
    axe.plot(datas[:,0],datas[:,1:],linewidth=2.0,color=colormaps)
    ...
    axe.axis(ymin=-3, ymax=3) # set y limits manually
    axe.set_yticks(range(-3,3))
    axe.set_title('Band Structure', fontsize=font['size'])
 ...
 #    axi.set_title('Band Structure (Down)', fontsize=font['size'])
    ...
    x1, y1 = readfile2(np.loadtxt("TDOS.dat"))
    # x2, y2 = readfile10(np.loadtxt("PDOS_SUM_UP.dat"))

    # ax1  = ax[0, 1]
    ax1.plot(y1, x1, linewidth=3.0)
    ...
    ax1.axhline(y=0, xmin=0, xmax=1, linestyle='--', linewidth=1.0, color='0.5')
